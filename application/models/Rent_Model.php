...
class Rent_Model extends CI_Model {
	...
	public function get_by_id_joined($id) {
		$this->db->select(array('rent.*','user.name as agent_name', 'user.email', 'user.phone', 'user.img_url as agent_img_url'));
		$this->db->from($this->table);
		$this->db->join('user', 'agent_id = user.id', 'left');
		// $this->db->join('city_area', 'city_id = category.id', 'left');
		$this->db->where('rent.id', $id);
		$query = $this->db->get();
		return $query->row();
	}
    ...
    public function fetch_rent_param($limit, $start, $param) {
    	$this->db->from($this->table);
    	if ($param['s_city']) {
    		$this->db->where('city', $param['s_city']);
    	}
