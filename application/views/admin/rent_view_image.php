	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-default">
				<h4 class="title_page"><?php echo $rent->subject ?></h4>
				<div class="panel-body">
					<p> Please crop image to (1060 x 50px) before upload</p>
		            <form action="<?php echo site_url("admin/rent/upload") ?>" id="form-upload"> 
		              <input type="hidden" value="<?php echo $rent_id ?>" name="id" id="input_id"/>            
...
<script type="text/javascript">
	...
	$('body').on('click', '.remove-file', function () {
		...
		$.ajax({
			url: uploadURI,
			type: 'post',
			data: {file_to_remove: me.attr('data-file'), id:"<?php echo ($rent_id) ?>"},
			success: function() {
	...
	function listFilesOnServer () {
		var inputID = $('#input_id')[0].value;
		var items = [];
		$.getJSON("<?php echo base_url('admin/rent/filelist/'.$rent_id.'/') ?>", function(data) {
			$.each(data, function(index, element) {
				if (element != "thumb") {
				var image = '<img class="lst_thmb" src="<?php echo base_url('assets/img/rent/'.$rent_id.'/') ?>'+ element +'">';
